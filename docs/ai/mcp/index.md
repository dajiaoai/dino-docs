...
| 工具名 | 作用 |
| :-- | :-- |
| `create_session` | 创建独立项目会话并返回 `sessionId`。除该工具外，其他工具均须传入此 `sessionId`，以保持编辑画板的连续性。 |
| `repl` | 在当前项目会话中按顺序执行 Dino Algeo REPL 命令，创建或修改定义、设置样式、管理标签和画板，并返回每条命令的执行结果。 |
| `import` | 使用结构化项目 JSON 或返回 JSON 的 HTTPS URL 替换当前项目会话内容，并返回已加载的画板数量。 |
| `load_template` | 从内联 JSON 或 HTTPS URL 加载母版，将其应用至指定画板、全部现有画板，或设为后续新建画板的默认母版。 |
| `export_image` | 将指定或当前画板渲染为 PNG 图片，并返回有效期 1 年的访问链接。 |
| `export_image_3d` | 将指定或当前 3D 画板渲染为 PNG 图片，并返回有效期 1 年的访问链接。 |
| `export` | 将当前项目会话导出为 `.algeo` 结构化 JSON 文件，并返回有效期 1 天的下载链接。 |
...
MCP 可直接使用的字体见[公开字体列表](/reference/fonts)。
